...
jsPsych.plugins["pointing"] = (function() {
    ...
    plugin.trial = function(display_element, trial) {
        ...
        // display stimulus
        var stimulus = this.stimulus(trial.grid, trial.grid_square_size,trial.target,trial.target_color);
        ...
        function showTarget(){
            var resp_targets;
            if(trial.allow_responses){
                resp_targets = display_element.querySelectorAll('.jspsych-serial-reaction-time-stimulus-cell2');
    ...
    plugin.stimulus = function(grid, square_size, target, target_color, labels) {
        ...
        for(var i=0; i<grid.length; i++){
            ...
            for(var j=0; j<grid[i].length; j++){
                ...
                if(grid[i][j] == 1) {
                    classname = 'jspsych-serial-reaction-time-stimulus-cell2';
                }
                ...
                //     stimulus += labels[i][j]
                // }
                if(target_color !== null && target_color === '#5cb85c' && target[0] == i && target[1] == j){
                    stimulus += '★';
                }
